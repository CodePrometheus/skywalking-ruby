...
require_relative '../../utils/grpc_client_interceptor'
      ...
      class GrpcClient
        class ManagementServiceGrpc
          def initialize(config)
            @config = config
            @management_service ||= Skywalking::V3::ManagementService::Stub.new(
          ...
          def report_instance_properties
            req = Skywalking::V3::InstanceProperties.new(
          ...
          def report_heartbeat
            begin
              req = Skywalking::V3::InstancePingPkg.new(
                service: @config.service_name,
                serviceInstance: @config.instance_name
              )
              @management_service.keep_alive(req)
            rescue ::GRPC::NotFound => e
              SkywalkingRuby.error "Error to connect APM: #{e.message}"
            end
          end
          ...
          def gen_service_instance
            ...
            begin
              ...
              p "Failed to get local environment information: #{e.message}"
        ...
        class TraceSegmentReportServiceGrpc
          def initialize(config)
            @trace_service ||= Skywalking::V3::TraceSegmentReportService::Stub.new(
              config.collector_backend_service,
              :this_channel_is_insecure,
            )
          end
          ...
          def report_segment(segment_obj)
            @trace_service.collect(segment_obj)
          end
        end
      end
    end
