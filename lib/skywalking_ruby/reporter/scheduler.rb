  ...
  module Reporter
    class Scheduler
      ...
      def dispatch(job, args)
        @jobs[job].each do |orig_job|
          begin
            ...
            p "Error in job #{job}: #{e.message}"
          end
        end
      end

      def subscribe(job_name, &job_func)
        trigger(:event_job, [job_name, job_func])
      end

      def trigger_timer(job_name, job_interval)
        trigger(:timer_job, Worker.new(job_name, job_interval))
      end
      ...
      class Worker
        ...
        def to_s
          "<Worker job_name: #{@job_name}, job_interval: #{@job_interval}, start_time: #{@start_time}>"
        end
